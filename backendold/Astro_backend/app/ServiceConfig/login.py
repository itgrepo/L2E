...
SERVER = 'outgoing.mail.go.th'  # PRODUCTION

# LINK = 'http://localhost:8080'  #DEVEVELOP
LINK = 'http://10.20.11.91'  #PRODUCTION
# LINK = ''  # PRODUCTION
### User And Password Mail ###
username_mail = 'adminbd@customs.go.th'
password_mail = 'P@ssw0rd'
###############################
...
and  expiration  >=  CURRENT_DATE and user.user_id = (select Max(bb.user_id) from user bb where bb.username = user.username)"""
        ...
        if len(result) == 0:
            conn = mysql.connect()
            cursor = conn.cursor()
            sql_username = "SELECT count_login, user_id FROM user WHERE username=%s AND status_account='active' ORDER BY user_id DESC LIMIT 1"
            ...
            if len(result_username) == 0:
                ...
                # if result_username[0]['count_login'] == 4:
                #     conn = mysql.connect()
                #     cursor = conn.cursor()
                #     sql = "UPDATE user SET status_id = 6, create_at = CURRENT_TIMESTAMP WHERE username = %s AND status_id != '7' AND status_account='active'"  # Update Status is Lock
                ...
                #     sql_insert_token = "INSERT INTO token_unlock_account VALUES (NULL, %s, %s, %s, %s, CURRENT_TIMESTAMP)"
                #     cursor.execute(sql_insert_token,
                #                    (token, username, email, 'active'))
                #     conn.commit()
                #     cursor.close()
                #     conn.close()
                #     ##--Check Email--##
                ...
                #             token, email, link, username, result_username[0]['user_id'])
                #         logAction(result_username[0]['user_id'], '/login', 'Please check in your email confirmation for unlock account', 'warning')
                #         return jsonify({"status": 'Please check in your email confirmation for unlock account'})
                #     else:
                #         #-Not have E-mail-#
                #         updatePassword(randomStringDigits(),
                #                        result_username[0]['user_id'])
                #         logAction(result_username[0]['user_id'], '/login', 'Please contact Administrator for unlock account', 'warning')
                #         return jsonify({"status": 'Please contact Administrator for unlock account'})
                #     ##---------------##
                # if  result_username[0]['count_login'] == 3:
                #     count_login = result_username[0]['count_login'] + 1
                #     conn = mysql.connect()
                #     cursor = conn.cursor()
                #     sql = "UPDATE user SET count_login = %s, create_at = CURRENT_TIMESTAMP WHERE username = %s AND status_id != '7' AND status_account='active'"  # Update count_login
                #     cursor.execute(sql, (count_login, username))
                #     conn.commit()
                #     cursor.close()
                #     conn.close()
                #     logAction(result_username[0]['user_id'], '/login', 'Locking account', 'warning')
                #     return jsonify({"status": 'Locking account'})
                ...
                cursor.close()
                conn.close()
                logAction(result_username[0]['user_id'], '/login', 'not found', 'warning')
...
def sendMailResetPassword(token, email, link, user_id):
    ...
    msg['From'] = fromaddr
    msg['To'] = toaddr
    msg['Subject'] = "Important - Reset Password FDepartment Operation Center"
    ...
    # body = "Password: " + random + "</br><p style='font-size: 14px;text-align:center' onClick='" + updatePassword(random, user_id) + "'>Click below to reset your password</p> <a href='" + str(link) + "' style='margin: 0 auto;display: block;width: 160px;height: 60px;margin-top: 30px;background-color: #19b5fe;text-align: center;line-height: 60px;color: #ffffff;border-radius: 4px;text-decoration: none;'>Reset Password</a>" + footer
    body = "<p style='font-size: 14px;' onClick='" + updatePassword(random, user_id) + "'>Hi&nbsp;&nbsp;" + firstname + "&nbsp;" + lastname + "<br><br>You have activated\"" + "Forget Password" + "\" request.<br> Please click \"" + "Reset Password" + "\" button below and use this temporary <b>password: </b><b>" + str(
        random) + "</b></p><p>to access the Department Operation Center.<br>Then we recommend you change the password right away for future security purpose. <br><br> If this was not your request, a malicious user may try to access your account. Please review your profile at <br> and change your password. If you need any help, you may contact Contact Center at<br>/contactcenter Tel: +66.</p><a href='" + str(link) + "' style='margin: 0 auto;display: block;width: 160px;height: 60px;margin-top: 30px;background-color: #19b5fe;text-align: center;line-height: 60px;color: #ffffff;border-radius: 4px;text-decoration: none;'>Reset Password</a>" + footer
