...
const categories = ref([
  { name: 'ข้อมูลประชากร', count: 2, active: false },
  { name: 'สาธารณสุข', count: 2, active: false },
  { name: 'การศึกษา', count: 1, active: false },
  { name: 'สวัสดิการสังคมและสิทธิมนุษยชน', count: 0, active: false },
  { name: 'แรงงาน', count: 0, active: false },
  { name: 'เศรษฐกิจ', count: 1, active: false },
  { name: 'คมนาคม', count: 0, active: false }
]);
      ...
      <div class="catalog-container">
        ...
        <section class="results-section">
          <div class="results-toolbar">
            ...
            <p v-else>พบทั้งหมด {{ datasets.length }} ชุดข้อมูล</p>
          ...
          <div v-if="paginatedDatasets.length > 0" class="datasets-list-vertical">
            <div v-for="ds in paginatedDatasets" :key="ds.id" class="ds-horizontal-card" @click="openDatasetDetail(ds)">
              <div class="ds-main-content">
                <div class="ds-header">
                  <h4 class="ds-title">{{ ds.title }}</h4>
                  <button class="btn-favorite" :class="{ 'is-active': isFavorite(ds) }" @click.stop="toggleFavorite(ds)">
                ...
                <div class="ds-footer">
                  <div class="ds-badges">
                    ...
                    <span v-for="f in ds.formats" :key="f" class="badge format">{{ f }}</span>
                  </div>
                  <div class="ds-meta">
          ...
          <div class="modal-body scrollbar-custom">
            ...
            <div v-if="activeTab === 'dictionary'" class="tab-content transition-fade">
              <table class="dictionary-table">
                ...
                <tbody>
                  <tr><td>1</td><td>id</td><td>Integer</td><td>รหัสรายการ</td></tr>
                  <tr><td>2</td><td>name</td><td>String</td><td>ชื่อรายการ</td></tr>
                  <tr><td>3</td><td>amount</td><td>Double</td><td>จำนวนเงิน</td></tr>
                  <tr><td>4</td><td>date</td><td>DateTime</td><td>วันที่บันทึก</td></tr>
                </tbody>
              </table>
            </div>

            <!-- Visual Tab -->
            <div v-if="activeTab === 'visual'" class="tab-content transition-fade" style="padding: 0;">
              <div v-if="selectedDataset?.external_dashboard_url" class="dashboard-container">
                <div class="dashboard-actions mb-4 p-4 flex justify-between items-center bg-slate-50 border-b border-slate-100">
                  ...
                  <a :href="selectedDataset.external_dashboard_url" target="_blank" class="flex items-center gap-2 px-4 py-2 bg-[var(--primary)] text-white rounded-lg hover:bg-emerald-700 transition-all font-semibold text-sm shadow-sm">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div v-if="activeTab === 'api'" class="tab-content transition-fade">
              <div v-if="selectedDataset?.api_enabled" class="api-info border border-slate-200 rounded-xl overflow-hidden shadow-sm bg-white">
