...
const approveUser = async (userId, username) => {
  ...
  try {
    const userData = getUserData();
    console.log('[Approve] User data from localStorage:', JSON.stringify(userData).substring(0, 100));
    const encodedUser = encodeUserData(userData);
    console.log('[Approve] Encoded user length:', encodedUser?.length);
    
    const payload = {
      user: encodedUser,
      target_user_id: userId
    };
    console.log('[Approve] Sending POST /approveUser with target_user_id:', userId);
    
    const response = await apiClient.post('/approveUser', payload);
    console.log('[Approve] Response:', JSON.stringify(response.data));
    ...
    if (response.data.status === 'success') {
      ...
      console.error('[Approve] Backend returned non-success:', response.data);
...
const rejectUser = async (userId, username) => {
  console.log(`[Reject] Starting rejection for user: ${username} (ID: ${userId})`);
  actionLoading.value = userId;
  errorMessage.value = '';
  try {
    const userData = getUserData();
    ...
    console.log('[Reject] Response:', JSON.stringify(response.data));
    ...
    if (response.data.status === 'success') {
      ...
      console.error('[Reject] Backend returned non-success:', response.data);
  ...
  <div class="page-layout">
    ...
    <main class="main-content">
      <div class="page-header">
        <div>
          <h1>อนุมัติสมาชิก</h1>
          <p class="header-subtitle">จัดการคำขอสมัครสมาชิกใหม่ที่รอการอนุมัติ</p>
        </div>
        <div class="header-actions">
          <button class="btn-refresh" @click="fetchPendingUsers">
          ...
          <button v-if="pendingUsers.length > 0" class="btn-approve-all" @click="approveAll">
