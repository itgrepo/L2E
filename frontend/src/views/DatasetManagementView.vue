...
const handleDelete = async (dataset) => {
    if (!confirm(`คุณแน่ใจหรือไม่ว่าต้องการลบชุดข้อมูล "${dataset.service_name}"?`)) return;
...
.stat-icon.active { background: var(--mso-pink-dark); color: var(--primary); }
...
.status-badge.active { background: var(--mso-pink-dark); color: var(--primary); }
...
.action-btn.toggle:hover { border-color: var(--primary); color: var(--primary); background: var(--mso-pink-dark); }
...
.selection-card.user .selection-icon { background: var(--mso-pink-dark); color: #166534; }
...
.item-action.add { background: var(--mso-pink-dark); color: var(--primary); }
.item-action.add:hover { background: var(--primary); color: white; }
