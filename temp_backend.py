...
def getDatasetApiEndpoints():
    try:
        ...
        sql = "SELECT service_name, api_endpoint, description, api_type, status, api_enabled FROM service WHERE dataset_id LIKE %s AND status = 'Active'"
...
def get_dataset_api(dataset_id):
    ...
    try:
        ...
        # 3. Handle Scopes (Row-Level Security) (DECOUPLED FROM api_type)
