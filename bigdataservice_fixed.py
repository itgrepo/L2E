...
def getSystemActivityLogs():
    ...
    try:
        ...
        sql = f"""
            SELECT l.log_id, l.user_id, u.username, u.email, org.org_name as organization, l.log_detail, l.path, l.type, l.ip, l.device, l.create_at 
            FROM log l
    ...
    try:
        ...
        if filter_status == 'All':
            sql = """SELECT r.request_id, r.user_id, r.service_id, r.fields_json, r.reason, r.status, r.created_at,
                            u.username, u.firstname, u.lastname, u.email, org.org_name as organization,
                            s.service_name, s.dataset_id
                     FROM dataset_permission_requests r
                     JOIN user u ON r.user_id = u.user_id LEFT JOIN organization org ON u.org_id = org.org_id
