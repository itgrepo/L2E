...
def get_dataset_api(dataset_id):
    ...
    try:
        ...
        if not svc_row:
            ...
            cursor.close()
            conn.close()
            return jsonify({'status': 'error', 'message': 'Service not found or inactive'}), 404
        ...
        if api_type != 'public':
            ...
            sql_cred = """SELECT c.credential_id, c.user_id, c.status, c.expires_at, u.previlage_id
                            ...
                            WHERE c.secret_key = %s AND c.service_id = %s"""
            cursor.execute(sql_cred, (apikey, real_service_id))
