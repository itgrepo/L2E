...
echo "Waiting for MariaDB to start (astro_db_uat)..."
MAX_TRIES=15
COUNT=0
while ! sudo docker exec astro_db_uat mysqladmin ping -u${MYSQL_USER:-astro} -p${MYSQL_PASSWORD:-password123} --silent; do
...
DB_CMD="sudo docker exec -i astro_db_uat mysql -u${MYSQL_USER:-astro} -p${MYSQL_PASSWORD:-password123} ${MYSQL_DATABASE:-psu_backend}"
